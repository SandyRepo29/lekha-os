...
import { complianceChatAction } from "@/backend/src/modules/compliance/actions";
import { Button } from "@/components/ui/button";

type Message = { role: "user" | "model"; text: string };

const QUICK_PROMPTS = [
  "What controls are missing for my frameworks?",
  "Which framework needs the most attention?",
  "Summarise my compliance posture",
  "What policies should I create next?",
  "Which gaps are most critical to fix?",
];

export function AiComplianceChat({ aiEnabled }: { aiEnabled: boolean }) {
  ...
  const send = (text: string) => {
    ...
    startTransition(async () => {
      const res = await complianceChatAction(text.trim(), messages);
      if (res.error) {
        setError(res.error);
  ...
  return (
    <div className="flex flex-col" style={{ minHeight: 400 }}>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4" style={{ maxHeight: 500 }}>
        ...
        {error && (
          <p className="rounded-xl border border-red-200 bg-red-100 px-3 py-2 text-sm text-red-700">
            {error}
          </p>
        )}
      ...
      <div className="border-t border-[var(--color-line)] p-4">
        ...
        >
          <input
            ...
            placeholder={
              aiEnabled
                ? "Ask about controls, gaps, frameworks…"
