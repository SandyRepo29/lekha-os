...
export const HELP_CONTENT: Record<string, ModuleHelp> = {
  ...
  "/policy-governance": {
    ...
    group: "Core GRC",
    overview:
      "Full policy lifecycle management — author, version, review, attest, and link policies to controls and frameworks with AI-powered policy health scoring.",
    features: [
      "Policy registry with version history and immutable snapshots",
      "Review workflow with reviewer assignments and outcome tracking",
      "Attestation campaigns — collect sign-offs with completion tracking",
      "Policy-to-control and policy-to-framework linkage",
      "Policy Health™ scoring based on review freshness and attestation completion",
      "AI policy summarisation and gap suggestions",
      "Policy status workflow: draft → under_review → approved → archived",
    ],
    tips: [
      "Schedule annual attestation campaigns in advance — the system tracks completion percentage automatically.",
      "Link policies to controls so Control Health™ picks up the policy-alignment signal.",
      "Approved policies with no review in 12 months trigger a Needs Review flag automatically.",
    ],
  ...
  "/contract-governance": {
    ...
    features: [
      ...
      "AI Contract Advisor — clause risk analysis, obligation extraction, executive summary, NL chat",
  ...
  "/workflow-studio": {
    ...
    group: "Privacy & Legal",
    overview:
      "Governance automation engine — define approval workflows, trigger automations, and orchestrate multi-step governance processes across the entire platform.",
    features: [
      "Workflow definition builder with trigger, conditions, and action steps",
      "Approval workflow routing with role-based assignment",
      "Workflow run history with status and duration tracking",
      "Multi-step sequences across modules (e.g. finding → issue → CAPA → closure)",
      "AI Workflow Generator — describe a process in plain English to generate a workflow",
      "Workflow templates for common governance processes",
      "Integration with Issue Hub™, Audit Management, and Policy Governance™",
    ],
    tips: [
      "Build a standard vendor-onboarding workflow so new vendors automatically trigger document requests and assessments.",
      "Use the AI generator to scaffold complex workflows — then refine the steps manually.",
      "Workflow runs are logged in Audit Logs for full traceability.",
    ],
