    ...
    <Card className="p-5">
      ...
      {messages.length === 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {SUGGESTIONS.map((s) => (
            <button key={s} onClick={() => send(s)} className="rounded-full border border-[var(--color-line)] px-3 py-1.5 text-xs hover:bg-white/[0.06] text-[var(--color-ink-dim)] transition-colors">
              {s}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-3 mb-4 max-h-80 overflow-y-auto">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${m.role === "user" ? "bg-[var(--color-blue)] text-white" : "bg-white/[0.06] text-[var(--color-ink)]"}`}>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-xl border border-[var(--color-line)] bg-white/[0.04] px-3 py-2 text-sm outline-none focus:border-[var(--color-blue)]/50"
        />
        <button
          onClick={() => send()}
