...
import { chatAction } from "@/backend/src/modules/regulatory-intelligence/actions";
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-bg-2)]/60 flex flex-col" style={{ minHeight: 400, maxHeight: 600 }}>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex items-center gap-3 rounded-xl border border-[var(--color-line)]/60 bg-white px-4 py-3">
          ...
          <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            ...
            <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-xs leading-relaxed ${
              m.role === "user"
                ...
                : "bg-[#F8F9FB] border border-[var(--color-line)]/60 text-[var(--color-ink-dim)]"
            }`}>
              {m.content}
            </div>
            {m.role === "user" && (
              <div className="h-7 w-7 shrink-0 grid place-items-center rounded-full bg-[#EEF2F7]">
        ...
        {pending && (
          <div className="flex gap-3">
            ...
            <div className="rounded-2xl border border-[var(--color-line)]/60 bg-[#F8F9FB] px-4 py-2.5 text-xs text-[var(--color-ink-faint)]">
      ...
      <div className="border-t border-[var(--color-line)] p-4">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-[var(--color-line)] bg-[#F8F9FB] px-3 py-2 text-xs text-[var(--color-ink)] placeholder:text-[var(--color-ink-faint)] focus:outline-none focus:ring-1 focus:ring-[var(--color-blue)]"
          />
          <button
            onClick={send}
