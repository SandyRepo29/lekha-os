...
import { AuditCapaReport } from "@/lib/reports/audit-capa-pdf";
...
export async function GET(
  ...
  try {
    ...
    const doc = React.createElement(AuditCapaReport, {
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="lekha-capas-${slug}-${date}.pdf"`,
      },
    });
  } catch (err) {
    console.error("[audit CAPAs PDF]", err);
