  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link href="/integration-hub/marketplace" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[var(--color-line)] text-sm font-medium hover:bg-white/[0.04] transition-colors">
          + Add Connection
        </Link>
      </div>
                ...
                {events.filter((e) => e.event.severity !== "critical").length > 0 && (
                  <div className="mt-3 space-y-1.5">
                    {events.filter((e) => e.event.severity !== "critical").slice(0, 3).map(({ event }) => (
                      <div key={event.id} className="flex items-start justify-between gap-2 rounded-lg bg-white/[0.03] border border-[var(--color-line)] px-3 py-2">
                        <div>
