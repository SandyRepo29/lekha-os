...
import { getApprovals, createApproval } from "@/backend/src/modules/toe/toe-service";
...
export async function GET(request: NextRequest) {
  ...
  const approvals = await getApprovals(ctx.orgId, {
  ...
  const paged = approvals.slice((page - 1) * pageSize, page * pageSize);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.request_type) return withRateLimitHeaders(err("request_type is required.", 400), rl);
    if (!body.title) return withRateLimitHeaders(err("title is required.", 400), rl);

    const approval = await createApproval(ctx.orgId, null, {
    ...
    return withRateLimitHeaders(ok(approval, 201), rl);
  } catch (e) {
    if (e instanceof DomainError) return withRateLimitHeaders(err(e.message, 422), rl);
    console.error("[POST /api/v1/operations/approvals]", e);
