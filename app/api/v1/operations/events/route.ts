...
import { getOrgEvents, getEventTypes, publishEvent } from "@/backend/src/modules/toe/toe-service";
...
export async function GET(request: NextRequest) {
  ...
  const events = await getOrgEvents(ctx.orgId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.event_type) return withRateLimitHeaders(err("event_type is required.", 400), rl);

    const event = await publishEvent(ctx.orgId, body.event_type, {
    ...
    return withRateLimitHeaders(ok(event, 201), rl);
  } catch (e) {
    if (e instanceof DomainError) return withRateLimitHeaders(err(e.message, 422), rl);
    console.error("[POST /api/v1/operations/events]", e);
