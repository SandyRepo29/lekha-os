...
import * as teamRepo from "@/backend/src/modules/team/team-repo";
...
import type { InvoiceWithPlan } from "@/backend/src/modules/billing/billing-repo";
...
export async function checkPlanLimit(
  ...
  if (resource === "assets" || resource === "storage_gb") {
    ...
    if (limit < 9999 && limit <= 10) {
      ...
      const { canUseFeature } = await import("@/backend/src/modules/billing/entitlements");
