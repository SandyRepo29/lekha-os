...
import * as svc from "@/lib/services/security-command-center/security-service";
...
export async function generateSecuritySummaryAction() {
  try {
    const session = await requireUser();
    const metrics = await (await import("@/lib/services/security-command-center/security-service")).getDashboardData(session.org?.id ?? "");
    const m = metrics.metrics;
    const readiness = (await import("@/lib/services/security-command-center/security-service")).computeSecurityReadiness(m);
    const ai = await import("@/lib/services/security-command-center/ai-security-service");
