...
> **✅ PRODUCTION-READINESS QA in progress — the standing charter, per-module playbook, and sign-off tracker are in §15 (Platform QA → Production-Readiness — Master Plan & Tracker). Discover group (Vendor Hub, Asset Intelligence, Contract Governance) complete. Evidence Vault signed off too — found 3 platform-wide bugs (Gemini output truncation, AiInsightPanel stale state, dark `<Select>`) that retroactively affect every module; resume one-by-one at Trust Exchange (Assess group).**
...
| **Gemini 2.5 Flash "thinking" can silently truncate output** | `generateText()`/`generateJSON()` in `lib/providers/ai/index.ts` set `thinkingConfig: { thinkingBudget: 0 }` on every call. Without it, the model can spend nearly the entire `maxOutputTokens` budget on internal reasoning tokens before writing the visible answer, returning `finishReason: MAX_TOKENS` with a response cut to a few words — found via a real repro where a 300-token budget produced `"Our ISO 27001:"` (287 tokens spent thinking, 9 on the actual answer). Any NEW raw `ai.models.generateContent()` call outside this wrapper (e.g. `lib/ai/gemini.ts`) must set the same `thinkingConfig` — these quick summary/extraction prompts don't need multi-step reasoning. |
